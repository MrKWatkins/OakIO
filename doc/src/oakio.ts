...
interface DotnetBuilder {
  withDiagnosticTracing(v: boolean): DotnetBuilder;
  withApplicationArgumentsFromQuery(): DotnetBuilder;
  withConfig(c: object): DotnetStarter;
}

interface DotnetStarter {
  create(): Promise<{ getAssemblyExports(n: string): Promise<unknown>; getConfig(): { mainAssemblyName?: string } }>;
}
